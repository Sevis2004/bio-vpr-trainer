...
function emptyAnswerFor(task) {
  switch (task.type) {
    case 'multipleChoice':
      return [];
    case 'dropdownGroup':
      return Object.fromEntries(task.items.map((item) => [item.id, null]));
    case 'dragToSlots':
      return Object.fromEntries(task.slots.map((slot) => [slot.id, null]));
    default:
      return null;
  }
}
