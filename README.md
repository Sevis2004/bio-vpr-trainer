...
> **Важно:** `data/variants/bio6_demo_2026.json` — тестовый вариант для проверки механики тренажёра. Это не реальный вариант ВПР/МЦКО и не должен использоваться как учебный или контрольный материал без замены содержания.
...
Build command не нужен. Все пути в проекте относительные (`./data/...`, `./src/...`), поэтому сайт корректно работает на GitHub Pages из подпапки репозитория, например `https://<user>.github.io/bio-vpr-trainer/`.
